...
export const GENERAL_LABEL_SCHEMA = "generic-v5";
...
const FILLERS: readonly IntentId[] = ["status_report", "plan", "explain", "share_feeling", "agree"];
const MAX_OPTIONS = 10;
const MIN_OPTIONS = 8;
const QUOTED = /“[^”]*”|「[^」]*」|『[^』]*』|‘[^’]*’|"[^"]*"|`[^`]*`/gu;
const QUESTION_CUE = /[？?]|(?:怎么|为什么|是否|是不是|能否|吗|呢)(?:[。！!，,\s]|$)|^(?:谁|什么|哪里|哪儿|何时|什么时候|几|多少)/u;
...
const CUES: ReadonlyArray<{ pattern: RegExp; ids: readonly IntentId[] }> = [
  { pattern: /不去了|不参加|没法去|我拒绝|不接受|我不同意/u, ids: ["reject", "distance"] },
  { pattern: /别再|不要再|先别|不想谈|到此为止|需要.{0,4}空间|保持距离/u, ids: ["set_boundary", "distance"] },
  ...
  { pattern: /别(?:太)?难过|别担心|不用怕|我(?:会|来)?陪你|我会陪你.{0,12}(?:处理|面对|度过)|慢慢来.{0,8}我陪你/u, ids: ["give_comfort"] },
  { pattern: /心里.{0,8}(?:难受|委屈|烦|苦)|有点(?:难过|委屈|伤心)|想跟你说|想聊聊|说说心里话|我很焦虑/u, ids: ["confide", "share_feeling"] },
  ...
  { pattern: /真厉害|做得好|太棒了|佩服你|真优秀|真好看/u, ids: ["praise", "show_affection"] },
  ...
  { pattern: /^(?:谢谢|谢了|感谢|辛苦了)|多谢你/u, ids: ["thank", "show_care"] },
  { pattern: /^(?:你好|嗨|哈喽|早安|早上好|晚安|晚上好|hello|hi)(?:[，,!！。\s]|$)/iu, ids: ["greet", "close_chat"] },
  { pattern: /拜拜|再见|回头聊|先这样|下次聊/u, ids: ["close_chat", "small_talk"] },
  ...
  { pattern: /帮我|帮忙|能不能.{0,8}(?:弄|看|改|做)|麻烦你|请你/u, ids: ["seek_help", "ask_question"] },
  { pattern: /不是.{0,30}而是|不对|应该是|我不同意|我不赞成/u, ids: ["correct", "clarify"] },
  ...
  { pattern: /后来呢|然后呢|你刚才说|再说说|所以呢/u, ids: ["follow_up", "ask_question"] },
  { pattern: /打算|计划|准备|明天.{0,8}(?:去|做)|下周.{0,8}(?:去|做)/u, ids: ["plan", "invite"] },
...
export function generalIntentQuestion(targetText: string): GeneralIntentQuestion {
  ...
  let matched = false;
  if (QUESTION_CUE.test(text) && !CARE_CONTINUATION.test(text)) {
    add("ask_question");
    matched = true;
  }
