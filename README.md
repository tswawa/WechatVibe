...
从 [Releases](https://github.com/tswawa/WechatVibe/releases) 下载 `WechatVibe-1.0.5-windows-x64.zip`。这是标准运行包，不内置 Laya 模型。解压后保留整个 `win-unpacked` 目录，双击其中的 `WechatVibe.exe`。需要本地分析时，在「设置 → 本地部署」点击下载模型，或选择已有的模型目录。模型作为独立附件提供，运行包不捆绑模型。
...
3. **选择模型**：在「设置 → 本地部署」下载 Laya，或选择已有的模型目录；从旧版升级后，原有模型会自动显示为已就绪。
